 ...
 * OrderServiceProxy是OrderService的子类，它的作用是将OrderService的每一个方法都封装成MethodMessage，
 * 然后提交给ActiveMessage队列，在使用OrderService接口方法的时候，实际上是在调用OrderServiceProxy中的方法。
 * 
 * OrderServiceProxy作为OrderService的一个实现，看上去与OrderService没多大关系，
 * 其主要作用是将OrderService接口定义的方法封装成MethodMessage，然后offer给ActiveMessageQueue。
 * 若是无返回值的方法，则只需要提交Message到ActiveMessageQueue中即可，但若是有返回值的方法，findOrderDetail是比较特殊的，
 * 它需要返回一个ActiveFuture，该Future的作用是可以立即返回，当调用线程获取结果时将进入阻塞状态。
 * @author 14378
 *
 */
